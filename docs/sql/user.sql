...
comment on column users.role is '用户角色(加着玩的)';

comment on column users.status_expire_time is '折扣过期时间(懒加载时检测)';

comment on column users.status is '用户状态 (1: 正常, 0: 禁用,896: 管理,2: 周卡,3: 小月卡,4: 大月卡,573: 金主,5: 可休息)';

comment on column users.source is '用户来源 (qq群号)';

comment on column users.invited_by is '邀请人qq号';

comment on column users.discount is '折扣(正常为1)';

comment on column users.total_time is '累计游玩时长(单位: 分)';

comment on column users.total_amount is '累计消费金额(单位: 元)';

comment on column users.play_count is '累计游玩次数';

comment on column users.last_come_time is '上次来玩时间';
